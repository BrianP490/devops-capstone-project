...
class TestAccountService(TestCase):
    ...
    def setUpClass(cls):
        ...
        init_db(app)
        talisman.force_https = False  # Prevents Talisman will force all requests to your REST API to use the https:// protocol
    ...
    def test_read_an_account(self):
        ...
        # Get the account that was just created
        new_account = response.get_json()  # Don't use .json() because the info received is serialized (python dictionary)
        self.assertEqual(new_account["name"], account.name)
    ...
    def test_cors_policye(self):
        """Test for the CORS Policies"""
        response = self.client.get('/', environ_overrides=HTTPS_ENVIRON)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        value = "*"
        self.assertEqual(response.headers.get("Access-Control-Allow-Origin"), value)